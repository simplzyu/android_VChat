...
import com.vchat.R;
import com.vchat.adapter.AddFriendAdapter;
...
public class AddFriendActivity extends Activity implements OnClickListener{
	...
	private void searchUser() {
		...
		new Thread(new Runnable() {
			@Override
			public void run() {
				listUser=XmppUtil.searchUsers(WeChatApplication.xmppConnection, search_content);
				if(listUser.size()>0){
				...
				new Thread(new Runnable() {
					...
					public void run() {
						try {
							...
							XmppUtil.addUsers(roster,toUser+"@"+ WeChatApplication.xmppConnection.getServiceName(), toUser,"我的好友");
							...
							XmppUtil.sendMessage(WeChatApplication.xmppConnection, message,toUser);
