...
import com.vchat.R;
import com.vchat.activity.ImgPageActivity;
...
public class ChatAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            hodler.fromIcon.setContent(hodler.fromText.toString());
        ...
        hodler.fromText.setOnLongClickListener(new onLongClicck(position));

        hodler.toText.setOnClickListener(new onClick(position, msg));
        hodler.toText.setOnLongClickListener(new onLongClicck(position));
        //图片点击
        hodler.fromImg.setOnClickListener(new onClick(position, msg));
        hodler.fromImg.setOnLongClickListener(new onLongClicck(position));
        hodler.toImg.setOnClickListener(new onClick(position, msg));
        hodler.toImg.setOnLongClickListener(new onLongClicck(position));
        //位置
        hodler.fromLocation.setOnClickListener(new onClick(position, msg));
        hodler.fromLocation.setOnLongClickListener(new onLongClicck(position));
        hodler.toLocation.setOnClickListener(new onClick(position, msg));
        hodler.toLocation.setOnLongClickListener(new onLongClicck(position));

        Msg message = new Msg();
        message.setContent(hodler.fromText.toString());
        message.setType("none");
        hodler.fromIcon.setOnClickListener(new onClick(0, message));
    ...
    class onClick implements OnClickListener {
        ...
        public void onClick(View arg0) {
            ...
            } else if(msg.getType().equals("none")){
                Toast.makeText(mContext,msg.getContent(),Toast.LENGTH_LONG).show();
            } else if(msg.getType().equals(Const.MSG_TYPE_TEXT)){
                Toast.makeText(mContext,msg.getContent(),Toast.LENGTH_LONG).show();
            }
     ...
     * 长按监听
     */
    class onLongClicck implements OnLongClickListener {
        int position;

        public onLongClicck(int position) {
            this.position = position;
        }
